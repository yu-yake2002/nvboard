...
VGA::VGA(SDL_Renderer *rend, int cnt, int init_val, int it, int ct)
    : Component(rend, cnt, init_val, it, ct),
      vga_screen_width(VGA_DEFAULT_WIDTH),
      vga_screen_height(VGA_DEFAULT_HEIGHT),
      vga_pre_clk(0),
      vga_pre_hsync(0),
      vga_pre_vsync(0),
      vga_pos(0),
  ...
  SDL_Texture *temp_texture = SDL_CreateTexture(
      rend, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STATIC,
...
VGA::~VGA() {
  SDL_DestroyTexture(textures_[0]);
  delete[] pixels;
}
...
void VGA::update_gui() {
  ...
  printf("%d frames\n", frames);
#endif
  SDL_Texture *temp_texture = textures_[0];
  SDL_Renderer *temp_renderer = renderer_;
  SDL_Rect *temp_rect = rects_[0];
  SDL_UpdateTexture(temp_texture, NULL, pixels,
...
void VGA::update_state() {
  ...
  if (vga_blank_n) {
    int vga_r = (output_map[VGA_R7] << 7) | (output_map[VGA_R6] << 6) |
                (output_map[VGA_R5] << 5) | (output_map[VGA_R4] << 4) |
                (output_map[VGA_R3] << 3) | (output_map[VGA_R2] << 2) |
                (output_map[VGA_R1] << 1) | output_map[VGA_R0];
    int vga_g = (output_map[VGA_G7] << 7) | (output_map[VGA_G6] << 6) |
                (output_map[VGA_G5] << 5) | (output_map[VGA_G4] << 4) |
                (output_map[VGA_G3] << 3) | (output_map[VGA_G2] << 2) |
                (output_map[VGA_G1] << 1) | output_map[VGA_G0];
    int vga_b = (output_map[VGA_B7] << 7) | (output_map[VGA_B6] << 6) |
                (output_map[VGA_B5] << 5) | (output_map[VGA_B4] << 4) |
                (output_map[VGA_B3] << 3) | (output_map[VGA_B2] << 2) |
                (output_map[VGA_B1] << 1) | output_map[VGA_B0];
...
void vga_set_clk_cycle(int cycle) { vga_clk_cycle = cycle; }

std::vector<Component *> VGA::Factory(SDL_Renderer *renderer, Json::Value &obj,
